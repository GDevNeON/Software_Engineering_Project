...
import Controler.SearchPhieuTra;
import BUS.PhieuTraBUS;
import DAO.PhieuTraDAO;
import DTO.PhieuTraDTO;
import GUI.add.addphieutra;
import GUI.details.detailsphieutra;
import GUI.details.cancelphieutra;
...
public class phieutra extends javax.swing.JPanel {
    ...
    ArrayList<PhieuTraDTO> list = new ArrayList<PhieuTraDTO>();
    ...
    public phieutra(int quyen) {
        initComponents();
        cbxAllDisplay();
        cbNccDisplay();
        cbHotenDisplay();
        searchDate();
        list = ptBUS.getAllPhieuTra();
        displaytoTable(list);
        displayAllPhieuTra();
    ...
    public void cbxAllDisplay() {
        ...
        int count = tblphieutra.getColumnCount();
        for (int i = 1; i < count; i++) {
            cbxAll.addItem("" + tblphieutra.getColumnName(i));
        }
    }

    public void cbNccDisplay() {
        cbncc.addItem("Tất cả");
        ArrayList<String> listncc = ptBUS.phieuTraDAO.selectAllNcc();
    ...
    public void cbHotenDisplay() {
        ...
        ArrayList<String> listhoten = ptBUS.phieuTraDAO.selectAllHoten();
    ...
    public void displaytoTable(ArrayList<PhieuTraDTO> list) {
        try {
            DefaultTableModel dt = (DefaultTableModel) tblphieutra.getModel();
            dt.setRowCount(0);
            for (PhieuTraDTO i : list) {
                if (i.getTrangthai() == 1) {
                    dt.addRow(new Object[]{
                        i.getMaphieutra(), i.getTenncc(), i.getTennt(), i.getThoigiantao(), i.getTongTien()
                    });
                }
            }
        } catch (Exception e) {
    ...
    public PhieuTraDTO getPhieuTraSelect() {
        int i_row = tblphieutra.getSelectedRow();
        PhieuTraDTO nv = ptBUS.phieuTraDAO.selectById(String.valueOf((int) tblphieutra.getValueAt(i_row, 0)));
        return nv;
    }
    ...
    public ArrayList<PhieuTraDTO> searchDate() {
        ...
        if (from != null && to != null) {
            ArrayList<PhieuTraDTO> allPhieuTra = ptBUS.phieuTraDAO.selectAll();
            for (PhieuTraDTO phieu : allPhieuTra) {
    ...
    public ArrayList<PhieuTraDTO> searchTien(long fromMoney, long toMoney) {
        ArrayList<PhieuTraDTO> result = new ArrayList<>();
        ArrayList<PhieuTraDTO> armt = PhieuTraDAO.getInstance().selectAll();
        for (var pt : armt) {
            long money = (long) pt.getTongTien();
    ...
    // Hàm hiển thị tất cả các phiếu trả
    public void displayAllPhieuTra() {
        ArrayList<PhieuTraDTO> allPhieuTra = ptBUS.phieuTraDAO.selectAll();
//        System.out.println(allPhieuTra);
        displaytoTable(allPhieuTra);
    }
    ...
    private void initComponents() {
        ...
        tblphieutra.setAutoResizeMode(javax.swing.JTable.AUTO_RESIZE_ALL_COLUMNS);
        tblphieutra.setRowHeight(50);
        tblphieutra.setSelectionBackground(new java.awt.Color(176, 212, 235));
        tblphieutra.setSelectionForeground(new java.awt.Color(10, 0, 0));
    ...
    private void txttimkiemKeyReleased(java.awt.event.KeyEvent evt) {// GEN-FIRST:event_txttimkiemKeyReleased
        ...
        switch (choose) {
            case "Tất cả":
                result = SearchPhieuTra.getInstance().searchTatCa(text);
                break;
            case "Mã phiếu nhập":
                result = SearchPhieuTra.getInstance().searchMaphieutra(text);
                break;
            case "Tên nhà cung cấp":
                result = SearchPhieuTra.getInstance().searchTenNCC(text);
                break;
            case "Nhân viên nhập":
                result = SearchPhieuTra.getInstance().searchTennvtra(text);
                break;
            case "Thời gian":
                result = SearchPhieuTra.getInstance().searchThoigiantao(text);
                break;
            case "Tổng tiền":
                result = SearchPhieuTra.getInstance().searchTongtien(text);
        }
    ...
    private void btnHuyActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnHuyActionPerformed
        // // TODO add your handling code here:
        if (!nqBUS.checkQuyen(listct, 12, "delete")) {
            btnThem.setBackground(Color.gray);
            JOptionPane.showMessageDialog(this, "Bạn không có đủ quyền hạn để sử dụng chức năng này !");
        } else {
            int selectedRow = tblphieutra.getSelectedRow();
            if (selectedRow == -1) {
                JOptionPane.showMessageDialog(this, "Vui lòng chọn phiếu trả muốn hủy");
            } else {
                cancelphieutra a = new cancelphieutra(this, (JFrame) SwingUtilities.getWindowAncestor(this), true);
                a.setVisible(true);

//                DefaultTableModel model = (DefaultTableModel) tblphieutra.getModel();
//
//                int output = JOptionPane.showConfirmDialog(this, "Bạn có chắc chắn muốn hủy phiếu trả ",
//                        "Xác nhận hủy phiếu trả", JOptionPane.YES_NO_OPTION);
//                if (output == JOptionPane.YES_OPTION) {
//                    ptBUS.deletePhieuTra((int) model.getValueAt(selectedRow, 0));
//                    JOptionPane.showMessageDialog(this, "Xóa thành công !");
//                    displaytoTable(ptBUS.phieuTraDAO.selectAll());
//                }
            }
        }
    ...
    private void cbnccItemStateChanged(java.awt.event.ItemEvent evt) {// GEN-FIRST:event_cbnccItemStateChanged
        ...
        if (choose.equals("Tất cả")) {
            result = ptBUS.phieuTraDAO.selectAll();
        } else {
            result = SearchPhieuTra.getInstance().searchTenNCC(choose);
        }
        displaytoTable(result);
    ...
    private void cbnvienItemStateChanged(java.awt.event.ItemEvent evt) {// GEN-FIRST:event_cbnvienItemStateChanged
        ...
        if (choose.equals("Tất cả")) {
            ...
            result = SearchPhieuTra.getInstance().searchTennvtra(choose);
        }
        displaytoTable(result);
    ...
    private void txttoMoneyKeyReleased(java.awt.event.KeyEvent evt) {// GEN-FIRST:event_txttoMoneyKeyReleased
        ...
        } else if (txtfromMoney.getText().isEmpty()
                && txttoMoney.getText().isEmpty()) {
            result = ptBUS.phieuTraDAO.selectAll();
        } else {
    ...
    private void txtfromMoneyKeyReleased(java.awt.event.KeyEvent evt) {// GEN-FIRST:event_txtfromMoneyKeyReleased
        ...
        // JOptionPane.showMessageDialog(this, "Không được để trống 1 trong 2 ô nhập số
        // tiền !");
        // } else if (txtfromMoney.getText().isEmpty() &&
        // txttoMoney.getText().isEmpty()) {
        // result = pnBUS.phieunhapDAO.selectAll();
    ...
    private void btnChiTietActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnChiTietActionPerformed
        // TODO add your handling code here:
        if (!nqBUS.checkQuyen(listct, 12, "view")) {
            btnThem.setBackground(Color.gray);
            JOptionPane.showMessageDialog(this, "Bạn không có đủ quyền hạn để sử dụng chức năng này !");
        } else {
            if (tblphieutra.getSelectedRow() == -1) {
                JOptionPane.showMessageDialog(this, "Vui lòng chọn phiếu trả để xem");
            } else {
                detailsphieutra a = new detailsphieutra(this, (JFrame) SwingUtilities.getWindowAncestor(this), true);
                a.setVisible(true);
            }
        }
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jButton1ActionPerformed
        ...
        ArrayList<PhieuTraDTO> searchResult = searchDate();
        ...
        if (from < 0) {
            ...
            JOptionPane.showMessageDialog(this, "Khoảng tiền không hợp lệ");
                ...
                || txttoMoney.getText().isEmpty()) {
            searchResult = ptBUS.phieuTraDAO.selectAll();
            JOptionPane.showMessageDialog(this, "Không được để trống 1 trong 2 ô nhập số tiền !");
        } else if (txtfromMoney.getText().isEmpty()
                && txttoMoney.getText().isEmpty()) {
            searchResult = ptBUS.phieuTraDAO.selectAll();
