...
public class PhieuXuatBUS {
    ...
    ArrayList<ChiTietPhieuXuatDTO> listCtpn;
    ...
    public Boolean add(PhieuXuatDTO pn) {
        boolean check = phieuXuatDAO.insert(pn) != 0;
        if (check) {
            this.listPhieuXuat.add(pn);
        }
        return check;
    }

    public Boolean addCtpn(ChiTietPhieuXuatDTO ctpn) {
        boolean check = ctPhieuXuatDAO.insert(ctpn) != 0;
        if (check) {
