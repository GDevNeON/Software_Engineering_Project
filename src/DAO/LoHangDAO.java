...
public class LoHangDAO implements DAOinterface<LoHangDTO> {
    ...
    public int delete(int maphieunhap, int masp) {
        int ketQua = 0;
        try {
            java.sql.Connection con = JDBC.getConnection();
            String sql = "DELETE FROM losanpham WHERE maphieunhap = ? AND masp = ?";
    ...
    public int UpdateForPhieuTra(int maphieunhap, int masp, int soluongtra) {
        int ketQua = 0;
        try {
            java.sql.Connection con = JDBC.getConnection();
            String sql = "UPDATE losanpham SET soluong = soluong - ? WHERE maphieunhap = ? AND masp = ?";
            PreparedStatement pst = con.prepareStatement(sql);
    ...
    public int getSoLuongTon(int maphieunhap, int masp) {
        int ketQua = 0;
        try {
            java.sql.Connection con = JDBC.getConnection();
            String sql = "SELECT soluong FROM losanpham WHERE maphieunhap = ? AND masp = ?";
            PreparedStatement pst = con.prepareStatement(sql);
    ...
    public void updateQuantity(int maphieunhap, int soluong) {
        ...
        try {
            ...
            // if (rowsUpdated > 0) {
            // System.out.println("Cập nhật thành công số lượng cho mã phiếu nhập: " +
            // maphieunhap);
            // }
        } catch (SQLException e) {
            e.printStackTrace();
