...
public class KhoHangDAO implements DAOinterface<KhoHangDTO> {
    ...
    public ArrayList<KhoHangDTO> selectAllSanPham() {
        ...
        try {
            Connection con = (Connection) JDBC.getConnection();
            String sql = " SELECT kv.makhuvuc, pn.maphieunhap, sp.masp, sp.tensp, lsp.soluong, lsp.gianhap, lsp.giaban, pn.thoigian" +
                        " FROM khuvuckho kv" +
                        " INNER JOIN phieunhap pn ON kv.makhuvuc = pn.makhuvuc" +
                        " INNER JOIN losanpham lsp ON pn.maphieunhap = lsp.maphieunhap" +
                        " INNER JOIN sanpham sp ON lsp.masp = sp.masp" +
            ...
            while (rs.next()) {
                ...
                kh.setSlton(rs.getInt("soluong"));
                kh.setGianhap(rs.getLong("gianhap"));
                kh.setGiaban(rs.getInt("giaban"));
                kh.setNgaynhap(rs.getDate("thoigian"));
                result.add(kh);
            }
            JDBC.closeConnection(con);
        } catch (Exception e) {
    ...
    public boolean checkExistingData(KhoHangDTO t) {
            KhoHangDTO result = null;
            try {
                  Connection con = (Connection) JDBC.getConnection();
                  String sql = "SELECT * from khuvuckho WHERE tenkhuvuc=?";
                  PreparedStatement pst = (PreparedStatement) con.prepareStatement(sql);
                  pst.setString(1, t.getTenkhuvuc());
                  ResultSet rs = (ResultSet) pst.executeQuery();
                  while (rs.next()) {
    ...
    public int getAutoIncrement() {
        ...
        try {
            Connection con = (Connection) JDBC.getConnection();
            String sql = "SELECT `AUTO_INCREMENT` FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'cuahangnuochoa' AND   TABLE_NAME   = 'khuvuckho'";
            PreparedStatement pst = (PreparedStatement) con.prepareStatement(sql);
