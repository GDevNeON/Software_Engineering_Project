...
//    }
    ...
    public ArrayList<PhieuNhapDTO> selectAll() {
        ...
        try {
            ...
            String sql = " SELECT phieunhap.maphieunhap, nhacungcap.tenncc, nhanvien.hoten, phieunhap.thoigian, phieunhap.tongtien FROM phieunhap"
                    ...
                    + " WHERE phieunhap.trangthai = 1 ORDER BY phieunhap.maphieunhap";
            PreparedStatement pst = (PreparedStatement) con.prepareStatement(sql);
    ...
    public ArrayList<String> selectAllMancc() {
        ...
        try {
            ...
            ResultSet rs = (ResultSet) pst.executeQuery();
            while (rs.next()) {
                String mancc = rs.getString("ma_nhacungcap");
                result.add(mancc);
            }
            JDBC.closeConnection(con);
        } catch (SQLException e) {
    ...
    public PhieuNhapDTO selectById(String t) {
        ...
        try {
            Connection con = (Connection) JDBC.getConnection();
            String sql = "SELECT * FROM phieunhap JOIN nhacungcap ON phieunhap.mancc = nhacungcap.mancc JOIN nhanvien ON nhanvien.manv = phieunhap.manv WHERE maphieunhap=?";
            PreparedStatement pst = (PreparedStatement) con.prepareStatement(sql);
            pst.setString(1, t);
            ResultSet rs = (ResultSet) pst.executeQuery();
            while (rs.next()) {
                ...
                Date thoigiantao = rs.getDate("thoigian");
                long tongtien = rs.getLong("tongtien");
                result = new PhieuNhapDTO(maphieu, mancc, tenncc, manv, tennvnhap, thoigiantao, tongtien);
            }
            JDBC.closeConnection(con);
        } catch (Exception e) {
        }
    ...
    public int getAutoIncrement() {
        ...
        try {
            Connection con = (Connection) JDBC.getConnection();
            String sql = "SELECT `AUTO_INCREMENT` FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'cuahangnuochoa' AND TABLE_NAME   = 'phieunhap'";
            PreparedStatement pst = (PreparedStatement) con.prepareStatement(sql);
    ...
    public int getAutoIncrementCtpn() {
        ...
        try {
            Connection con = (Connection) JDBC.getConnection();
            String sql = "SELECT `AUTO_INCREMENT` FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'cuahangnuochoa' AND TABLE_NAME   = 'ctphieunhap'";
            PreparedStatement pst = (PreparedStatement) con.prepareStatement(sql);
            ResultSet rs2 = pst.executeQuery(sql);
            if (!rs2.isBeforeFirst()) {
                System.out.println("No data");
    ...
    public ArrayList<PhieuNhapDTO> searchByDate(Date fromDate, Date toDate) {
        ArrayList<PhieuNhapDTO> result = new ArrayList<>();
        try {
            Connection con = (Connection) JDBC.getConnection();
