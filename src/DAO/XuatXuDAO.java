...
public class XuatXuDAO implements DAOinterface<XuatXuDTO> {
    ...
    public ArrayList<XuatXuDTO> selectAll() {
        ...
        try {
            ...
            while (rs.next()) {
                int mathuonghieu = rs.getInt("maxuatxu");
                String tenthuonghieu = rs.getString("tenxuatxu");

                XuatXuDTO lh = new XuatXuDTO(mathuonghieu, tenthuonghieu);
                result.add(lh);
            }
            JDBC.closeConnection(con);
        } catch (Exception e) {
    ...
    public int getAutoIncrement() {
        ...
        try {
            Connection con = (Connection) JDBC.getConnection();
            String sql = "SELECT `AUTO_INCREMENT` FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'cuahangnuochoa' AND TABLE_NAME = 'xuatxu'";
            PreparedStatement pst = (PreparedStatement) con.prepareStatement(sql);
