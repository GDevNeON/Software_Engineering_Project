...
public class PhieuXuatDAO implements DAOinterface<PhieuXuatDTO> {
    ...
    public int insert(PhieuXuatDTO t) {
        ...
        try {
            ...
            String sql = "INSERT INTO `phieuxuat`(`maphieuxuat`, `thoigian`, `tongtien`, `soluong`, `manv`, `makh`, `trangthai`) VALUES (?,?,?,?,?,?,1)";
...
//    public int insertCtpx(PhieuXuatDTO t) {
//        int result = 0;
//        try {
//            Connection con = (Connection) JDBC.getConnection();
    ...
    public ArrayList<PhieuXuatDTO> selectAll() {
        ...
        try {
            ...
            String sql = " SELECT phieuxuat.maphieuxuat, khachhang.tenkhachhang, nhanvien.hoten, phieuxuat.thoigian, phieuxuat.tongtien FROM phieuxuat"
                    ...
                    + "                                   WHERE phieuxuat.trangthai = 1 ORDER BY phieuxuat.maphieuxuat";
            PreparedStatement pst = (PreparedStatement) con.prepareStatement(sql);
            ...
            while (rs.next()) {
                ...
                PhieuXuatDTO phieuxuat = new PhieuXuatDTO(maphieu, tenkh, tennvnhap, thoigiantao, tongtien,1);
...
// bao sua select by id
    ...
    public int getAutoIncrement() {
        ...
        try {
            Connection con = (Connection) JDBC.getConnection();
            String sql = "SELECT `AUTO_INCREMENT` FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'cuahangnuochoa' AND TABLE_NAME   = 'phieuxuat'";
            PreparedStatement pst = (PreparedStatement) con.prepareStatement(sql);
    ...
    public ArrayList<PhieuXuatDTO> searchByDate(Date fromDate, Date toDate) {
        ArrayList<PhieuXuatDTO> result = new ArrayList<>();
        try {
            Connection con = (Connection) JDBC.getConnection();
            String sql = "SELECT * FROM phieuxuat WHERE thoigian BETWEEN ? AND ?";
            PreparedStatement pst = (PreparedStatement) con.prepareStatement(sql);
            ...
            while (rs.next()) {
                ...
                Date thoigiantao = rs.getDate("thoigian");
                long tongtien = rs.getLong("tongtien");
                int makm=rs.getInt("giamgia");
                PhieuXuatDTO phieuxuat = new PhieuXuatDTO(maphieu, tenkh, tennvnhap, thoigiantao, tongtien,makm);
                result.add(phieuxuat);
            }
            JDBC.closeConnection(con);
        } catch (SQLException e) {
